...
namq_10_gdp <- get_eurostat("namq_10_gdp", stringsAsFactors = FALSE)
                           # filters = list(geo %in% c("LT", "EE", "LV")), 
                            #na_item = "D1", 
                            #s_adj = "SCA", 
                            # time >= "2000-01-01", 
                            #unit = "PC_GDP") bandziau taip, bet neveike
...
df <- namq_10_gdp %>% filter(geo %in% c("LT", "EE", "LV"), 
                            na_item == "D1", 
                            s_adj == "SCA", 
                             time >= "2000-01-01", 
                            unit == "PC_GDP")
...
```{r}
mean <- ddply(df, .(geo), summarize, values=mean(values))

#LT<- namq_10_gdp %>% filter(geo == "LT", 
             #               na_item == "D1", 
             #               s_adj == "SCA", 
             #                time >= "2000-01-01", 
              #              unit == "PC_GDP")
#LTmean <- mean(LT$values)


#EE<- namq_10_gdp %>% filter(geo == "EE", 
#                            na_item == "D1", 
#                            s_adj == "SCA", 
 #                            time >= "2000-01-01", 
  #                          unit == "PC_GDP")
#EEmean <- mean(EE$values)


#LV<- namq_10_gdp %>% filter(geo == "LV", 
 #                           na_item == "D1", 
  #                          s_adj == "SCA", 
   #                          time >= "2000-01-01", 
    #                        unit == "PC_GDP")
#LVmean <- mean(LV$values)
...
ggplot(df, aes(x=time, y=values, col=geo))+
        geom_line()+
        labs(title = "Darbo pajamu ir BVP santykio grafikas", 
             subtitle = "eurostat", x= "Metai", y= "santykis")
```
